    ...
    def simulate(self):
        ...
        while True:
            ...
            try:
                ...
                self.array_price = [self.house_profile_DF.at[
                                        self.count_row, "energy_market_price"]] + self.energy_price_DF.iloc[
                                                                                  self.count_row, 1:13].to_list()
                """
                self.array_price = []
        ...
        with open(info_filename, "w") as file_object:
            ...
            csv.writer(file_object).writerow(
                [self.home.p, self.home.teta, self.home.gamma, self.home.epsilon, self.loops])
        info_filename = os.path.join(self.directory, "device_info.csv")
        with open(info_filename, "w") as file_object:
            csv.writer(file_object).writerow(
                ["device_id", "state_number", "action_number", "beta", "max_capacity", "min_energy_demand",
                 "max_energy_demand", "deficit", "energy_demand"])
            print_list = []
            for device in self.device_list:
                ...
                if type(device) == NSL_Battery:
                    print_list.append(
                        [device.id, "-", "-", "-", device.max_capacity, "-", "-", "-", device.energy_demand])
                if type(device) == Naif_Battery:
                    print_list.append(
                        [device.id, "-", "-", "-", device.max_capacity, "-", "-", device.deficit, device.energy_demand])
                if type(device) == Controlable_load:
                    print_list.append([device.id, device.state_number, device.action_number, device.beta, "-",
                                       device.min_energy_demand, device.max_energy_demand, "-", "-"])
                if type(device) == CL_Battery:
                    print_list.append(
                        [device.id, device.state_number, device.action_number, device.beta, device.max_capacity,
                         device.min_energy_demand, device.max_energy_demand, "-", "-"])
