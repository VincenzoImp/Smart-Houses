...
class SL_Battery(Shiftable_load):
    ...
    def __init__(self, simulation, id, k, T_ne, state_number, max_capacity, current_state_of_charge = 0, T_ini = 0, T_end = 23, energy_demand=0, column_info=None, plots_directory="", is_active=False):
        #T_ini, T_w, T_end devono rispettare i vincoli descritti nell'articolo e dovrebbero matchare con working_hours
        super().__init__(simulation, id, k, T_ne, state_number, T_ini, T_end, energy_demand, column_info, plots_directory, is_active)
...
def insert_SL_Battery(simulation):
    ...
    while True:
        ...
        new_battery = SL_Battery(simulation, device_id, k, 0, state_number, max_capacity, 0, 0, 0, energy_demand, ("PEV_input_state_of_charge", "PEV_hours_of_charge"))
